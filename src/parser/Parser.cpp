...
namespace jc::parser {
    ...
    struct_field_list Parser::parseStructFields() {
        ...
        while (not eof()) {
            ...
            fields.emplace_back(makeBoxNode<StructField>(std::move(id), std::move(type), closeSpan(begin)));
        }
    ...
    ident_pr Parser::justParseIdent(const std::string & panicIn) {
        ...
        return Ok(Ident(token, closeSpan(begin)));
    }
    ...
    ident_pr Parser::parseIdent(const std::string & expected) {
        ...
        if (maybeIdToken.some()) {
            return Ok(Ident(maybeIdToken.unwrap("parseIdent -> maybeIdToken"), span));
        }
